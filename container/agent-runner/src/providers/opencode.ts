...
function spawnOpencodeServer(config: Record<string, unknown>, timeoutMs = 10_000): Promise<{ url: string; proc: ChildProcess }> {
  return new Promise((resolve, reject) => {
    ...
    const port = 4096;
    // Write config as a file — OPENCODE_CONFIG_CONTENT is not supported in 1.4.x.
    // OpenCode reads from $XDG_CONFIG_HOME/opencode/opencode.json at startup.
    const xdgDataHome = process.env.XDG_DATA_HOME ?? path.join(process.env.HOME ?? '/home/node', '.local', 'share');
    const opencodeConfigDir = path.join(xdgDataHome, 'opencode');
    fs.mkdirSync(opencodeConfigDir, { recursive: true });
    fs.writeFileSync(path.join(opencodeConfigDir, 'opencode.json'), JSON.stringify(config, null, 2));
