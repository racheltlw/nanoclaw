...
/**
 * Exported hook: resolves phone numbers to LIDs via WhatsApp and persists them.
 * Called on connect (for all contacts without LIDs) and when a new contact is added.
 * No-ops if the adapter isn't connected yet.
 */
let _discoverLidsImpl: ((phones: string[]) => Promise<void>) | null = null;

export async function discoverLidsForPhones(phones: string[]): Promise<void> {
  if (_discoverLidsImpl) await _discoverLidsImpl(phones);
}
...
registerChannelAdapter('whatsapp', {
  factory: () => {
    ...
    // Cooldown to prevent hammering onWhatsApp when many LID-only messages arrive
    let lastOnDemandDiscoveryMs = 0;
    const ON_DEMAND_DISCOVERY_COOLDOWN_MS = 60_000;

    // Wire the module-level discovery hook to this socket instance.
    // Queries one phone at a time so we always know which phone maps to which LID.
    _discoverLidsImpl = async (phones: string[]) => {
      if (!connected || phones.length === 0) return;
      for (const phone of phones) {
        try {
          const results = await sock.onWhatsApp(phone);
          const result = results?.[0];
          if (!result?.exists) continue;
          const phoneJid = `${phone.replace('+', '')}@s.whatsapp.net`;
          if (result.jid.endsWith('@lid')) {
            // WhatsApp returned the LID JID directly for this phone number
            const lidUser = result.jid.split('@')[0].split(':')[0];
            setLidPhoneMapping(lidUser, phoneJid);
            log.info('Discovered LID via onWhatsApp', { phone, lid: lidUser });
          } else {
            // Phone JID returned — check for a lid field in the result
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            const lid = (result as any).lid as string | undefined;
            if (lid) {
              const lidUser = lid.split('@')[0].split(':')[0];
              setLidPhoneMapping(lidUser, result.jid.endsWith('@s.whatsapp.net') ? result.jid : phoneJid);
              log.info('Discovered LID via onWhatsApp (lid field)', { phone, lid: lidUser });
            } else {
              log.info('No LID for phone — will match by phone JID', { phone, returnedJid: result.jid });
            }
          }
        } catch (err) {
          log.info('LID discovery failed for phone', { phone, err });
        }
      }
    };
    ...
    function setLidPhoneMapping(lidUser: string, phoneJid: string): void {
      ...
      // Persist LID to the contact record so LID-based lookups survive restarts
      const contact = lookupContactAny(phoneJid);
      if (contact && !contact.lid) {
        try {
          updateContactLid(contact.id, lidUser);
          log.info('Persisted LID mapping to contact', { lidUser, phone: contact.phone_e164, contactId: contact.id });
        } catch (err) {
          log.warn('Failed to persist LID mapping to contact', { lidUser, phone: contact.phone_e164, err });
        }
      }
    }
    ...
    async function translateJid(jid: string): Promise<string> {
      ...
      // Check contacts table for a persisted LID → phone mapping
      const contact = lookupContact(jid);
      if (contact) {
        const phoneJid = `${contact.phone_e164.slice(1)}@s.whatsapp.net`;
        setLidPhoneMapping(lidUser, phoneJid);
        log.info('Resolved LID from contacts table', { lidJid: jid, phoneJid });
        return phoneJid;
      }
    ...
    async function connectSocket(): Promise<void> {
      ...
      sock.ev.on('connection.update', (update) => {
        ...
        if (connection === 'close') {
          ...
          // Discover LIDs for contacts that don't have one yet
          void discoverLidsForPhones(listContactsWithoutLid().map((c) => c.phone_e164));
      ...
      // Contact sync — learn LID ↔ phone mappings for all contacts on startup
      sock.ev.on('contacts.upsert', (updatedContacts) => {
        for (const contact of updatedContacts) {
          ...
          if (lidUser) setLidPhoneMapping(lidUser, contact.id);
        }
      });
      ...
      sock.ev.on('messages.upsert', async ({ messages }) => {
        for (const msg of messages) {
          try {
            ...
            // On-demand LID discovery: when a DM arrives as an unresolved @lid JID,
            // run discoverLidsForPhones for contacts that still lack a LID, then retry.
            // This catches the case where startup discovery didn't establish the mapping.
            if (chatJid.endsWith('@lid')) {
              const now = Date.now();
              if (now - lastOnDemandDiscoveryMs > ON_DEMAND_DISCOVERY_COOLDOWN_MS) {
                lastOnDemandDiscoveryMs = now;
                const withoutLid = listContactsWithoutLid();
                if (withoutLid.length > 0) {
                  log.info('On-demand LID discovery triggered', { unresolved: chatJid, count: withoutLid.length });
                  await discoverLidsForPhones(withoutLid.map((c) => c.phone_e164));
                  chatJid = await translateJid(rawJid);
                }
              }
            }

            // pushName fallback: when onWhatsApp returns only a phone JID (no LID field),
            // automatic discovery never establishes the LID→phone mapping. Use the sender's
            // WhatsApp display name to look up the contact and cache the mapping so this
            // and future messages from the same device route correctly.
            if (chatJid.endsWith('@lid') && msg.pushName) {
              const contact = findContactByPhoneOrName(msg.pushName);
              if (contact) {
                const phoneJid = `${contact.phone_e164.slice(1)}@s.whatsapp.net`;
                const lidUser = rawJid.split('@')[0].split(':')[0];
                setLidPhoneMapping(lidUser, phoneJid);
                chatJid = phoneJid;
                log.info('LID resolved by pushName', { pushName: msg.pushName, lidJid: rawJid, phoneJid });
              }
            }
            ...
            // Skip bot's own outbound messages to prevent echo loops, but allow
            // the user's own messages (fromMe=true, not a bot reply) to route.
            ...
            // Whitelist gate — only whitelisted contacts and the owner can reach the agent.
            // For 1:1 DMs the sender IS the chatJid; in groups use the participant JID.
            // lookupContactAny handles both phone JIDs and LID JIDs internally.
            const senderJid = isGroup ? sender : chatJid;
            const effectiveContact = lookupContactAny(senderJid);
            const ownerBypass = !effectiveContact && isOwnerJid(senderJid);

            if (!ownerBypass) {
              if (!effectiveContact) {
                log.debug('WhatsApp message dropped: sender not in contacts whitelist', { senderJid });
                continue;
              }
              if (effectiveContact.status === 'paused') {
                log.debug('WhatsApp message dropped: contact is paused', {
                  senderJid,
            ...
            // Inject contact display info: override sender with the stored display_name
            // so the agent sees a readable name, not a raw JID. Append team if set.
            // Owner bypass falls back to the WhatsApp push name.
            const contactLabel = effectiveContact
              ? effectiveContact.team
                ? `${effectiveContact.display_name} (${effectiveContact.team})`
                : effectiveContact.display_name
              : senderName;
